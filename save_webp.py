...
# webp_quality            = 91
  ...
  def INPUT_TYPES(self):
    return {
      ...
      'hidden': {
        ...
        'foldername_prefix': self.foldername_prefix,
        'foldername_keys': self.foldername_keys,
        'delimiter': self.delimiter,
        'counter_digits': self.counter_digits, 
        'save_job_data': self.save_job_data,
        'job_data_per_image': self.job_data_per_image,
  ...
  def get_latest_counter(self, one_counter_per_folder, folder_path, filename_prefix, counter_digits=2, counter_position='last', output_format='.webp'):
    ...
    if not os.path.exists(folder_path):
        print(f"SaveWebp {version} error: Folder {folder_path} does not exist, starting counter at 1.")
        return counter
    ...
    try:
        ...
        print(f"SaveWebp {version} error: An error occurred while finding the latest counter: {e}")
            ...
            if '' not in splitKey:
              ...
              if splitKey[0].isdecimal():
                ...
                if node in prompt:
                  ...
                  print(f"SaveWebp info: node #{node} not found")
  ...
  def save_job_to_json(self, save_job_data, prompt, filename_prefix, positive_text_opt, negative_text_opt, job_custom_text, resolution, output_path, filename, timestamp=datetime.now()):
    ...
    if os.path.exists(json_file_path):
      try:
        ...
        print(f"SaveWebp {version} error: The file {json_file_path} is empty or malformed. Initializing with empty data.")
        existing_data = {}
...
# class SaveWebp -------------------------------------------------------------------------------
  ...
  def save_images(self,
    ...
    full_output_folder, _, _, _, _ = folder_paths.get_save_image_path(custom_filename, self.output_dir, images[0].shape[1], images[0].shape[0])
