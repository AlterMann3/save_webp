...
#        'preview': ('BOOLEAN', {'default': True, 'label_on': 'On', 'label_off': 'Off'}),  # Buton şeklinde preview ayarı
      },
      'optional': {
      },
      'hidden': {
  ...
  def get_latest_counter(self, one_counter_per_folder, folder_path, filename_prefix, counter_digits=2, counter_position='last', output_format='.webp'):
    ...
    try:
        ...
        print(f"SaveWebpAvif {version} error: An error occurred while finding the latest counter: {e}")
            ...
            if '' not in splitKey:
              ...
              if splitKey[0].isdecimal():
                ...
                if node in prompt:
                  ...
                  print(f"SaveWebpAvif info: node #{node} not found")
  ...
  def save_job_to_json(self, save_job_data, prompt, filename_prefix, positive_text_opt, negative_text_opt, job_custom_text, resolution, output_path, filename, timestamp=datetime.now()):
    ...
    if os.path.exists(json_file_path):
      try:
        ...
        print(f"SaveWebpAvif {version} error: The file {json_file_path} is empty or malformed. Initializing with empty data.")
        existing_data = {}
